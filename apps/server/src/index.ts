...
app.post<{Body:{id?:string;name?:string}}>('/api/users',async(req,reply)=>{
 ...
 const id=req.body?.id||randomUUID(); let user=store.users.find(u=>u.id===id);
 if(!user){user={id,name,createdAt:new Date().toISOString()};store.users.push(user)}else user.name=name; await persist(); return user;
});
...
io.on('connection',socket=>{
 ...
 socket.on('message:send',async(input:Partial<Message>,ack?:(r:any)=>void)=>{
  ...
  const message:Message={id:input.id||randomUUID(),chatId:input.chatId,senderId:input.senderId,text:String(input.text).slice(0,10000),createdAt:input.createdAt||new Date().toISOString(),delivered:true,read:false};
  if(!store.messages.some(m=>m.id===message.id)){store.messages.push(message);await persist()}
  io.to(`chat:${message.chatId}`).emit('message:new',message);socket.emit('message:delivered',{messageId:message.id});ack?.({ok:true,message});
 });
 ...
 socket.on('call:signal',({targetUserId,...payload})=>{for(const [sid,uid] of sockets)if(uid===targetUserId)io.to(sid).emit('call:signal',payload)});
